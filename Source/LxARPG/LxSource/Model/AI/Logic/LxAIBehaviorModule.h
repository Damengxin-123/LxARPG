...
#include "LxAIBehaviorModule.generated.h"

class ALxAICharacter;
class AAIController;
class ULxCharacterBehaviorControlComponent;

/** 将 AI 决策结果转换为角色通用移动、近战和技能调用的 UObject 模块。 */
UCLASS(BlueprintType, Blueprintable, EditInlineNew, DefaultToInstanced, DisplayName="AI行为执行模块")
class LXARPG_API ULxAIBehaviorModule : public ULxAIControlModuleBase
{
	GENERATED_BODY()

public:
	/** 缓存所属 AI 角色、出生位置和角色运动组件。 */
	virtual void InitializeModule(ULxAIControlComponent* InOwnerComponent) override;
