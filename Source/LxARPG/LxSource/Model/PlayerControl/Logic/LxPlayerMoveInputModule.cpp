...
void ULxPlayerMoveInputModule::InitializeModule(ULxPlayerControlComponent* InOwnerComponent)
{
...
void ULxPlayerMoveInputModule::HandleInputValue(ELxInputActionID InInputActionID, FLxInputValue InValue)
{
	if (!m_pBehaviorControlComponent)
	{
		m_pOwnerCharacter = Cast<ALxBaseCharacter>(GetOwner());
		m_pBehaviorControlComponent = m_pOwnerCharacter
			? m_pOwnerCharacter->GetCharacterBehaviorControlComponent()
			: nullptr;
	}
	if (!m_pBehaviorControlComponent)
	{
